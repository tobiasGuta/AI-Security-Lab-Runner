...
const (
	ScratchRoot        = "/scratch"
	MaxCookieSizeBytes = 1048576 // 1MB
)
...
// ValidatePath validates that targetPath is strictly under /scratch and returns relative path components.
func ValidatePath(targetPath string, requireFile bool) (string, []string, error) {
	if targetPath == "" {
...
func GenerateTempFileName() string {
	b := make([]byte, 8)
	_, _ = rand.Read(b)
	return filepath.Join(ScratchRoot, fmt.Sprintf(".sys_tmp_%s.tmp", hex.EncodeToString(b)))
}
